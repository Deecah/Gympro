    ...
    <div class="d-flex">
        ...
        <div class="flex-grow-1 p-4 bg-light">
            ...
            <c:if test="${empty schedules}">
                <div class="alert alert-info">No workouts scheduled for this week. Click "Add" to create a new workout.</div>
            </c:if>

            <a class="btn btn-success mb-4" href="${pageContext.request.contextPath}/ScheduleServlet?trainerId=${trainerId}">Back to list</a>
                            ...
                            <td class="time-slot" key="${dayDate}">
                                    ...
                                    <c:if test="${not empty schedule.date && schedule.date.toLocalDate() eq dayDate && not empty schedule.exercises && fn:length(schedule.exercises) > 0}">
                                        ...
                                        <div class="workout-box">
                                            ...
                                            <c:choose>
                                                ...
                                                <c:otherwise>
                                                    <c:choose>
                                                        <c:when test="${now.toString() gt schedule.date.toString()}">
                                                        ...
                                                        <form action='${pageContext.request.contextPath}/TimetableServlet' method="post">
                                                            ...
                                                            <input type="hidden" name="workoutId" value="${schedule.workoutId}">
                                                            <input type="hidden" name="status" value="completed">
                                                            <button type="submit" class="btn btn-sm btn-outline-primary">Hoàn thành</button>
                                                        </form>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </c:otherwise>
                                ...
                                <c:choose>
                                    <c:when test="${now.toString() le dayDate}">
                                        <span class="badge bg-primary font-bold" onclick="showWorkoutModal('${dayDate}')">Add &#43;</span>
                                    </c:when>
                                    <c:otherwise>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="workoutForm" action='${pageContext.request.contextPath}/TimetableServlet' method="post">
                                ...
                                <div class="mb-3">
                                    ...
                                    <c:if test="${empty exercisePrograms}">
                                        <div class="alert alert-warning">No exercises available for this program.</div>
                                    </c:if>
                                    <c:forEach var="exerciseProgram" items="${exercisePrograms}">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="exerciseProgramIds" value="${exerciseProgram.exerciseProgramID}" id="exercise_${exerciseProgram.exerciseProgramID}">
                                            <label class="form-check-label d-flex flex-column border border-1 border-primary rounded-2 py-2 px-3" for="exercise_${exerciseProgram.exerciseProgramID}">
                                                <span class="exercise-meta">
                                                    <strong>${exerciseProgram.exerciseName}</strong>
                                                </span>
                                                <div class="d-flex justify-content-between m-0">
                                                    <span class="exercise-meta">
                                                        🏋️ Sets: <strong>${exerciseProgram.sets}</strong><br>
                                                        🔁 Reps: <strong>${exerciseProgram.reps}</strong><br>
                                                        ⏱️ Rest: <strong>${exerciseProgram.restTimeSeconds}s</strong>
                                                    </span>
                                                    <span class="exercise-extra">
                                                        💪 Muscle: <em>${exerciseProgram.muscleGroup}</em><br>
                                                        🧰 Equipment: <em>${exerciseProgram.equipment}</em>
                                                    </span>
                                                </div>
                                            </label>
                                        </div>
